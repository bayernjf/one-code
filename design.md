...
- 最短工作时长门槛（`minWorkDuration`，短于此时长的任务不发完成通知）
- 通知方式（声音 / 系统通知 / 仅失焦时）
```
...
> 实际落地与此略有偏差：探针没有独立成包，而是留在 `packages/desktop/src/probes/`——它们依赖 Electron 侧的运行时，单独成包只会多一层无人复用的边界。真实结构见 `handoff.md` 的项目结构一节。
...
- [x] 多工作区目录的自动发现策略 —— 阶段 1b 已落地 `workspaceDiscovery.ts`，自动发现最近活跃工作区，用户可在设置页覆盖
...
- [~] Windows / Linux 支持优先级 —— Windows 已适配（进程探针 `tasklist`、shell hook 只列 bash、CI 含 windows-latest、NSIS 安装包），但未在真实 Windows 上跑过；Linux 仍未动
- [x] 独立 App 的自动更新方案 —— electron-updater（完整事件流 + IPC + 设置页更新 UI），随 release 工作流发布
- [—] 阶段 4b：浏览器扩展访问 localhost socket 的权限 / CORS 限制 —— 随 4b 搁置，桌面端已由 Codex 探针覆盖
...
- socket 安全：本机其他进程可能伪造信号 → 已用 token 文件（0600）+ `timingSafeEqual` 比对；同用户进程仍可读 token，本机同用户信任模型下接受。
...
- 2026-08-26：文档对账。校正与代码脱节处：core 用例数 20→22、`npm run test:unit` 实际已改名 `npm test`、`sourceName.ts` 与 `notificationCoordinator.ts` 补进结构树、进程探针注明双平台命令。§9 补注探针未独立成包的实际取舍；§10 勾掉已解决项（工作区自动发现、自动更新、socket 鉴权），Windows 标为「已适配未实测」。同时记录一处真实缺口：`minWorkDuration` 已持久化生效但设置页未暴露入口。
- 2026-08-26：补上 `minWorkDuration` 设置页入口。桌面设置页「灵敏度」区域新增输入框（0~600 秒，0 表示不设门槛），渲染进程读写该字段，主进程保存后同步调用 `aggregator.setMinWorkDuration()`。至此该参数全链路可 UI 配置，不再需要手改 userData 下的 JSON。
- 2026-08-26：多会话状态管理落地。aggregator 从单状态机重构为 `Map<MonitorSource, SessionState>`，全局状态按 waiting > working > done > idle 聚合，每来源独立防抖与门槛，无破坏性 API 变更。托盘新增活跃会话子菜单与并发数标题。同日完成统计仪表盘（今日/本周/累计 + 7 天趋势 + 工具分布）与历史窗口来源筛选。
